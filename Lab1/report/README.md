...
Скрипт `scripts\generate_report_materials.py` генерирует данные, которые можно вставить в отчет по лабораторной работе: параметры RSA, значения AES, результаты проверки подписей, демонстрацию CRC32 и hex dump. Файл шифрования и файлы подписей сохраняются как ASN.1 DER-контейнеры.
